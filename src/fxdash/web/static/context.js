...
export function briefingHtml(brief) {
  ...
  const catchup = brief.mode === 'catchup';
  const warningText = w => ({
  ...
  return `<section class="brief-preview col gap14" data-briefing-state="${esc(brief.state || 'preview')}" data-briefing-mode="${esc(brief.mode)}"><h2 class="sec">${catchup ? copy('Catch-up briefing','补发简报') : formal ? copy('Morning briefing','文字晨报') : copy('Text briefing preview','文字简报预览')} ${esc(brief.date || '')}</h2>
    ...
    ${catchup ? `<p class="hint">${copy('Generated after the morning target, using reporting collected at the times above. This does not count as an on-time 09:00 edition.','本期在晨间时点之后生成，新闻按上方实际时间采集，不计作 09:00 按时出刊。')} ${esc(brief.generated_at)}</p>` : brief.late_publication ? `<p class="hint">${copy('This edition was finalized late.','本期延迟生成。')} ${esc(brief.generated_at)}</p>` : ''}
