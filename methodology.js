...
<p>The News page and email lead with a short market recap: the dollar's broad direction, up to two standout percentage moves and a point to watch. One substantial model gap or short checked news item may add context. The original figures, factor contributions and residuals remain in Details and sources. Code derives the recap from the frozen edition without another language-model call. Azure reads the same recap, usually in about half a minute to a minute. Text remains available if speech generation fails.</p>
...
<p>News 页和邮件先给一段简短复盘，交代美元整体方向、最多两个主要涨跌幅和后续关注点。明显的模型解释缺口或简短的已检查新闻可以补充背景。原始数字、因子贡献和残差保留在“详情与来源”中。代码从当期存档生成复盘，不再调用语言模型，Azure 朗读同一份内容，通常约半分钟到一分钟。语音生成失败时，文字仍可阅读。</p>
